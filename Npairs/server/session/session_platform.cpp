...
namespace Session
{
    ...
    SessionPlatform::SessionPlatform(boost::asio::io_context& ioContext, boost::asio::ip::tcp::socket &&socket)
    ...
    void SessionPlatform::start(OnReadCallback onReadCallback, OnBrokenCallback onBrokenCallback) 
    {
        ...
        boost::asio::post(readStrand_, 
                        std::bind(&SessionPlatform::read, shared_from_this()));
    }

    void SessionPlatform::stop(OnDisConnectedCallback callback) 
    {
        onDisConnectedCallback_ = callback;
        boost::asio::post(writeStrand_,
                        std::bind(&SessionPlatform::issueDisConnect, shared_from_this()));
    }
    ...
    void SessionPlatform::read() 
    {
        ...
        if(isAsyncReadCompleteHandlerCalled_) 
        {
            ...
            boost::asio::async_read(socket_,
                                    ...
                                    boost::asio::bind_executor(readStrand_,std::bind(&SessionPlatform::onRead, shared_from_this(), std::placeholders::_1, std::placeholders::_2)));
        }
    }

    void SessionPlatform::onRead(boost::system::error_code error, std::size_t bytesTransferred) 
    {
        ...
        if(!error) 
        {
            ...
            Frame::FramePlatform frame(incoming_);
    ...
    void SessionPlatform::moveFromQueueAndWrite() 
    {
        if (isAsyncWriteCompleteHandlerCalled_) 
        {
            ...
            if (outgoing_.size()) 
            {
                ...
                boost::asio::async_write(socket_,
                                        ...
                                        boost::asio::bind_executor(writeStrand_, std::bind(&SessionPlatform::onWrite, shared_from_this(), std::placeholders::_1, std::placeholders::_2)) );
            }
        }
    }

    void SessionPlatform::onWrite(boost::system::error_code error, std::size_t bytesTransferred) 
    {
