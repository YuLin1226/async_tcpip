#include "session_ex.hpp"
#include <iostream>

namespace Session
{
    
    SessionEx::SessionEx(boost::asio::io_context& ioContext, boost::asio::ip::tcp::socket &&socket)
    ...
    void SessionEx::start(OnReadCallback onReadCallback, OnBrokenCallback onBrokenCallback) 
    {
        ...
        boost::asio::post(readStrand_, 
                        std::bind(&SessionEx::read, shared_from_this()));
    }

    void SessionEx::stop(OnDisConnectedCallback callback) 
    {
        onDisConnectedCallback_ = callback;
        boost::asio::post(writeStrand_,
                        std::bind(&SessionEx::issueDisConnect, shared_from_this()));
    }

    void SessionEx::write(unsigned char *pStart, size_t dataSize) 
    {
        ...
        Frame::FrameEX frame(outgoingQueue_);
        frame.writeFrameContent(pStart, dataSize);
        boost::asio::post(writeStrand_,
                        std::bind(&SessionEx::moveFromQueueAndWrite, shared_from_this()));
    }

    void SessionEx::issueDisConnect() 
    {
    ...
    void SessionEx::read() 
    {
        ...
        if(isAsyncReadCompleteHandlerCalled_) 
        {
            ...
            boost::asio::async_read(socket_,
                                    ...
                                    boost::asio::bind_executor(readStrand_,std::bind(&SessionEx::onRead, shared_from_this(), std::placeholders::_1, std::placeholders::_2)));
        }
    }

    void SessionEx::onRead(boost::system::error_code error, std::size_t bytesTransferred) 
    {
    ...
    void SessionEx::moveFromQueueAndWrite() 
    {
        if (isAsyncWriteCompleteHandlerCalled_) 
        {
            ...
            if (outgoing_.size()) 
            {
                ...
                boost::asio::async_write(socket_,
                                        ...
                                        boost::asio::bind_executor(writeStrand_, std::bind(&SessionEx::onWrite, shared_from_this(), std::placeholders::_1, std::placeholders::_2)) );
            }
        }
    }

    void SessionEx::onWrite(boost::system::error_code error, std::size_t bytesTransferred) 
    {
